...
/*
  ...
  Adapted from a provided reference. One behavioural change: the whole WebGL
  stack is skipped when the visitor asks for reduced motion. This is a
  full-viewport, permanently-moving background, which is exactly the case
  that setting exists for — and skipping init rather than freezing a frame
  also spares the GPU the work entirely.
*/
...
class FluidSim {
  ...
  update(time: number) {
...

    {
      ...
      (u.force.value as THREE.Vector2).set((mouse.diff.x / 2) * mf, (mouse.diff.y / 2) * mf);
      (u.center.value as THREE.Vector2).set(clampedX, clampedY);
      (u.scale.value as THREE.Vector2).set(cs, cs);
...
export function PixelLiquidBg({
  ...
  return (
    <div ref={mountRef} className={cn("relative h-full w-full overflow-hidden", className)} {...props}>
