...
// Dark mode's ambient GlowingWave runs warm (orange), so the existing
// site-blue accent (matches --syntax-func / CircuitBoard's "active" node)
// already reads as a genuine complementary contrast against it — confirmed
// visually, left as-is. Light mode's wave runs the opposite direction
// (vivid blue), so a blue-family flame there sits too close in hue to pop;
// use the site's own warm accent (--syntax-number) instead so the flame
// contrasts against the wave the same way dark mode's does, rather than
// blending into it.
const ACCENT_DARK: [number, number, number] = [130 / 255, 170 / 255, 255 / 255]; // #82AAFF
const ACCENT_LIGHT: [number, number, number] = [234 / 255, 88 / 255, 12 / 255]; // #EA580C
...
export function FlameWrap({
  ...
  // Explicit `color` always wins; otherwise derive from the current theme
  // so the flame stays on-brand across the light/dark toggle.
  const resolvedColor = color ?? (isDark ? ACCENT_DARK : ACCENT_LIGHT);
  const mergedOptions: FlameWrapOptions = { ...options, color: resolvedColor };

  const sourceRef = useRef<HTMLCanvasElement>(null);
  const contentRef = useRef<HTMLDivElement>(null);
  const outputRef = useRef<HTMLCanvasElement>(null);
  const instanceRef = useRef<FlameWrapInstance | null>(null);
  const [initialOptions] = useState(mergedOptions);
  ...
  const reach = Math.round(Math.max(mergedOptions.height ?? 170, 24) * 1.5) + 40;
  const glow = Math.round(Math.max(mergedOptions.spread ?? 8, 8) * 3) + 16;
