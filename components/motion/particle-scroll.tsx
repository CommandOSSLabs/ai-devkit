...
// Pasted verbatim from the Canvas UI "Particle Scroll" doc — a WebGL2
// dissolve-to-sand scroll effect. Where the browser implements the
// experimental canvas `layoutsubtree` + `drawElementImage` API it captures
// the wrapped content into a texture and animates it; everywhere else
// `native` is false and, in the vendor original, the WebGL canvas
// contributes nothing at all (base pass draws at alpha 0, particle pass
// skipped) — no evergreen browser ships that API today, so in practice
// nothing ever happened on scroll. useFallbackReveal below is the addition:
// an IntersectionObserver + CSS transition standing in for the GPU
// particle field on every browser that isn't `native`, so there's a real,
// visible "assembles as it scrolls into view" effect regardless of
// platform support for the experimental path.
...
// Cross-browser stand-in for the GPU dissolve when `native` is false (i.e.
// on every evergreen browser today). Same idea as the shader version —
// content assembles as the formation line reaches it, and dissolves back
// once it's scrolled past — done as an opacity/blur/translate transition
// toggled by IntersectionObserver instead of per-pixel sand, since that
// needs no experimental canvas API to actually run.
function useFallbackReveal(
  contentRef: React.RefObject<HTMLDivElement | null>,
  active: boolean,
  { point, band, settle, fade }: Required<ParticleScrollOptions>,
) {
  useEffect(() => {
    if (!active) return;
    ...
    root.style.setProperty("--ps-duration", `${Math.max(settle, 0.1)}s`);
    root.style.setProperty("--ps-rise", `${Math.round(22 * fade)}px`);

    // rootMargin is resolved in px against the root's own box on the fly by
    // the browser, but `point` is a fraction and `band` a px runway around
    // it — mixing a % and a px length in one rootMargin side isn't legal,
    // so both get folded into one px value up front using the root's own
    // (already-known, since it's the scroll container) height.
    const h = root.clientHeight;
    const linePx = Math.min(Math.max(point, 0), 1) * h;
    const bottomMargin = -Math.max(h - linePx - Math.max(band, 0), 0);
      ...
      (entries) => {
        for (const entry of entries) {
          ...
          if (entry.isIntersecting) {
            el.classList.add("ps-reveal", "ps-revealed");
          } else {
            el.classList.add("ps-reveal");
            el.classList.remove("ps-revealed");
          }
        }
      },
      { root, rootMargin: `0px 0px ${bottomMargin}px 0px`, threshold: 0 },
    );
    ...
    return () => {
      io.disconnect();
      sections.forEach((section) => section.classList.remove("ps-reveal", "ps-revealed"));
    };
  }, [active, contentRef, point, band, settle, fade]);
}
...
export interface ParticleScrollProps extends ParticleScrollOptions {
  ...
  /** id applied to the actual scrolling element, so outside code (e.g. a "scroll to top" handler) can target it directly instead of `window`. */
  contentId?: string;
}
