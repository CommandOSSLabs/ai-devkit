...
// Adapted from a provided CodeBlock reference. Two changes were required to
// run here: the original is written for Tailwind v4 (`bg-(--cb-bg)`), and this
// project is on v3.4, so every custom-property utility is rewritten as
// `bg-[var(--cb-bg)]`; and v3 can't apply an opacity modifier to a CSS
// variable colour, so the alpha tints are precomputed into their own vars
// instead of `.../12` suffixes.
...
function hexToHsl(hex: string): [number, number, number] {
  ...
  if (value.length === 4 || value.length === 8) value = value.slice(0, value.length === 4 ? 3 : 6);
  if (value.length === 3) {
    value = value
      .split("")
      .map((c) => c + c)
...
function buildTheme(accent: string, mode: "dark" | "light" = "dark") {
  ...
  const accentTone = dark ? tint(Math.min(Math.max(l, 56), 70)) : tint(Math.min(Math.max(l, 38), 50));
...
function CopyButton({ code, floating }: { code: string; floating?: boolean }) {
  ...
  const copy = useCallback(async () => {
    // The async API can exist and still reject (permissions, unfocused
    // document), so the legacy path is a fallback for *failure*, not just
    // for absence — otherwise copy silently does nothing in those cases.
    const legacyCopy = () => {
      try {
...
export function CodeBlock({
  ...
  const safeLanguage = typeof language === "string" && language.length > 0 ? language : "text";
  ...
  const highlighted = useMemo(() => new Set(Array.isArray(highlightLines) ? highlightLines : []), [highlightLines]);

  const cssVars = {
    "--cb-accent": colors.accent,
    "--cb-accent-soft": colors.accentSoft,
    "--cb-accent-ring": colors.accentRing,
    "--cb-bg": colors.bg,
  ...
  return (
    ...
    >
      ...
      <div
        ...
        tabIndex={0}
        className={cn(
          "min-h-0 flex-1 overflow-auto outline-none [scrollbar-color:var(--cb-border)_transparent] [scrollbar-width:thin] selection:bg-[var(--cb-selection)] focus-visible:ring-2 focus-visible:ring-[var(--cb-accent-ring)]",
          showFrame && "py-3",
        )}
      >
        <Highlight code={trimmed} language={safeLanguage} theme={theme}>
          {({ tokens, getLineProps, getTokenProps }) => {
            ...
            return (
              <pre data-slot="code-block-pre" className="w-max min-w-full font-mono text-[13px] leading-6 [tab-size:4]">
                {tokens.map((line, i) => {
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        showFrame && "px-3.5",
                        highlighted.has(i + 1) && "bg-[var(--cb-line-wash)]",
                        lineProps.className,
                      )}
                    >
                      {showLineNumbers && (
                        <span
                          aria-hidden
                          className="mr-4 shrink-0 select-none text-right text-[var(--cb-gutter)]"
