...
void main() {
  ...
  float mist = (1.0 - smoothstep(0.0, max(uHaloWidth, 0.0005), edge)) * uGlow * uHalo;
...
export default function GlowingWave({
  ...
  return (
    <div
      ref={shell}
      className={["glowing-wave", className].filter(Boolean).join(" ")}
    >
      <div className="glowing-wave-canvas">
        <Canvas
          orthographic
          dpr={ceiling}
      ...
      {children ? <div className="glowing-wave-content">{children}</div> : null}
    </div>
  );
}
